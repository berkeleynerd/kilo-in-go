...
func TcSetAttr(fd uintptr, termios *Termios) error {
	...
	// syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), uintptr(syscall.TIOCGETA), uintptr(unsafe.Pointer(&termios)))

	if _, _, err := syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), uintptr(syscall.TIOCSETA+1), uintptr(unsafe.Pointer(&termios))); err != 0 {
		return err
	}
...
func TcGetAttr(fd uintptr) *Termios {
	...
	if _, _, err := syscall.Syscall(syscall.SYS_IOCTL, uintptr(fd), uintptr(syscall.TIOCGETA), uintptr(unsafe.Pointer(termios))); err != 0 {
